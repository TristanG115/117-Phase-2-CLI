        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list):
                    self.logger.warning(
                        f"HF API returned list for {self.model_id}, expected dict"
                    )

                    if len(data) == 0:
                        self.logger.error(
                            f"Empty list returned from HF API for {self.model_id}"
                        )
                        return {}
                    ...
                    for item in data:
                        ...
                        if item_id == self.model_id:
                            self.logger.info(
                                f"Found exact match in list for {self.model_id}"
                            )
                            data = item
                            break
                    else:
                        # No exact match, use first item if it's a dict
                        if isinstance(data[0], dict):
                            self.logger.warning(
                                f"No exact match for {self.model_id}, using first item: "
                                f"{data[0].get('id', 'unknown')}"
                            )
                            data = data[0]
                        else:
                            self.logger.error(
                                f"First item in list is not a dict: {type(data[0])}"
                            )
                            return {}

                elif not isinstance(data, dict):
                    # Unexpected type
                    self.logger.error(f"Unexpected data type from HF API: {type(data)}")
                    return {}
